...
func buildSimpleTestNetwork(inputCount, hiddenCount, outputCount int, activationFunction string) *core.Network {
	...
	network.AddInputLayer(inputCount).
		...
		AddOutputLayer(outputCount, activationFunction)
...
func Test_accumulateHiddenDeltas_setsTheCorrectValues(t *testing.T) {
	...
	expected := net.GetLayer(1).Nodes()[0].Output() * net.OutputLayer().Nodes()[0].Delta() * net.GetLayer(1).Nodes()[0].Connection(0).Weight()
	assert.InDelta(t, expected, net.GetLayer(1).Nodes()[0].Delta(), floatDelta)
}
...
func Test_accumulateGradients_setsTheCorrectValues(t *testing.T) {
	...
	expected := net.OutputLayer().Nodes()[0].Delta() * net.GetLayer(1).Nodes()[0].Output()
	assert.InDelta(t, expected, net.GetLayer(1).Nodes()[0].Connection(0).Gradient(), floatDelta)
}
